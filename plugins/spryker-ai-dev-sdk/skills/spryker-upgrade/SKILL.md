...
| 66 | **Phantom regression from a contended test run.** Running the suite while an asset build, a `cache:empty`, or HTTP smoke requests hit the same stack produces failures that do not reproduce in isolation — and against a baseline they look exactly like upgrade damage | re-run each suspect suite **alone** before believing it. On the validation run 4 "new failures" (and a 146-assertion drop at identical test count) all passed individually | run the comparison suite **sequentially and with nothing else touching the stack**. Never report a regression from a run that shared the environment; the cost of a re-run is far below the cost of chasing a fiction |
