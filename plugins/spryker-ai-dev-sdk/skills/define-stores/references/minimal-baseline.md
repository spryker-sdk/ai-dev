# Minimal bootable baseline (verified: playbook experiment E1, green boot)
...
- `payment_method.csv`; `shipment.csv`, `shipment_type.csv`, `shipment_method_shipment_type.csv`; `warehouse.csv`, `warehouse_address.csv`.
...
- **`product_label.csv`** → include exactly the labels required by wired `ProductLabelDependencyProvider` updater plugins that have NO self-installer in `InstallerDependencyProvider`. Current demoshop: **`NEW`** (ProductNew) + **`SALE`** (Pyz ExampleProductSalePage). The other two labels (`Alternatives available`, `Discontinued`) self-install — do NOT add them. Missing NEW/SALE → install abort. Both are dynamic (no product SKUs needed).
...
- `cms_block.csv` → **keep ONLY the `cms-block-email--*` rows** (~54). These are transactional email bodies rendered by the Mail module via `renderCmsBlockAsTwig` — dropping them throws on every registration/order/password mail (proven E4.4). Drop all other (home/nav/catalog demo) blocks.
  - **Caveat (B2/B5): keeping only the email rows ALSO deletes all ~21 home/nav/catalog blocks AND every content-BLOCK template definition** — block templates live in the `template_name`/`template_path` columns of `cms_block.csv` rows (`@CmsBlock/template/*`), NOT in `cms_template.csv` (which is for CMS *pages*, `@Cms/templates/*`). So the homepage renders empty (the home template emits `{% cms_slot 'slt-3' %}`/`'slt-5'` etc., and `cms_slot` produces no wrapper markup when a slot resolves to nothing — an empty page with no clue). **Whoever runs after this MUST author replacement blocks + their block templates** (`title_and_content_block.twig`, `banner_block.twig`, `banner_grid_column_block.twig`, `section_block.twig`, `jumbotron_block.twig`, `navigation_block.twig`, `product-cms-block.twig`) plus `cms_slot_block` wiring and per-store `cms_block_store`.
  - **Locales (B7): the kept email blocks ship `en_US`/`de_DE` only.** With project locales other than those, run `duplicate-columns --from en_US --to <project locales>` on `cms_block.csv` — a REQUIRED step whenever the email blocks are kept, or every transactional email body silently falls back to English.
  - **Inherited (F3): the shipped data has 55 email blocks but only 50 `cms_block_store.csv` rows** — 5 email blocks are unassigned to any store. This is harmless shipped data; do NOT spend time reconciling 55 vs 50 as if it were your own error.
...
`category_store` (root only; children inherit), `cms_block_store` (the kept email blocks × store), `cms_page_store` (header-only, pages dropped), `payment_method_store`, `shipment_method_store`, `shipment_type_store`, `warehouse_store`. Detect the store column from each header (`store`, `store_name`, or `included_store_names`).
