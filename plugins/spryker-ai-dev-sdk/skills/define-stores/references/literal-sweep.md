...
- **`de_DE` (or `en_US`) left in the picker on a project with no content for it is the same trap** — it renders empty product lists too. When you add the override, offer to drop the unused defaults from the returned set (and mirror that in `TRANSLATION_ZED_FALLBACK_LOCALES`, above).
