...
- List the real alternatives when the answer is a small closed set (DB engine, which apps to disable [multiSelect], mail engine).
...
3. **Services** — the clone already ships every service and application **enabled**; ask what to **turn off or swap**, never what to "enable". Accepting the shipped set as-is is valid. Read the actual `deploy.dev.yml` first — present what is really there, not a memorised list. Phrase engine/app choices per the plain-language table — the operator hears "the database, search and cache your shop runs on — keep the recommended defaults unless your hosting needs otherwise" and, for apps, a plain "what turning this off means for your shop", not the bare engine/app names as jargon.
   - **Infra engines are NEVER disablable — only swappable.** The shop cannot run without a database, a search engine, a broker or a key-value store, so **never phrase them as "disable or replace"**: they are a **read-only confirmation** with one escape hatch — a swap the customer's *hosting* mandates. Only the **`dev_services_disabled`** set (next bullet) is genuinely disablable. The engines: database (`mariadb` shipped — the Cloud-recommended default; warn on a swap), search (`opensearch` shipped; may swap to Elasticsearch), broker (`rabbitmq`), key-value store + session (`valkey`). Record **only** swaps, and only after checking the alternative is actually supported in `deploy.dev.yml`'s schema — offering an engine the deploy file can't take is worse than not asking. Failure signature: an option reading "disable search" or "turn off the key-value store". **Show the PHP image tag read-only** (read it from `deploy.dev.yml` `image:`, e.g. `spryker/php:8.4`) — changing it is out of scope for this wizard; if the customer environment pins a different PHP, flag it as a follow-up rather than editing.
...
4. **Stores** — **ask stores first; the region is proposed after** (the developer need not know regions up front). **Store mode is NOT a question** — `store_mode: dms` is a fixed value written straight to the state file: nothing downstream supports classic mode (`define-stores` *deletes* the classic-mode `dynamic-store-off` variants), so the only supportable answer is DMS. Never surface it.
   ...
   - **State the store code's business consequence BEFORE they pick it:** the code appears in **every storefront URL** as `/<STORE>/<lang>/…`, and in the deploy/env tokens — it is permanently visible to shoppers. **Short uppercase codes (2–4 chars), like the shipped `DE`/`AT`/`US`, are the convention;** longer codes work and ship as-is (one run picked a 13-char code and every URL became `/WATERDELIVERY/uk/…`). Say this in the question, not afterwards.
...
6. **Catalog scope (adapt mode only — skip for clean/generate/leave; optional, default: keep the whole demo catalog)** — ask whether the project sells the entire demo catalog or only part of it. Most projects keep everything (the demo catalog is a placeholder they'll replace later). Default: keep all. This drives `project-data`'s **reduce** strategy, which runs **pre-boot** (so the first boot imports the reduced set — no reset).
   - **Derive the options from the data BEFORE asking: read `category.csv`, count the products under each top-level branch, and offer the branches BY NAME WITH PRODUCT COUNTS** as a `multiSelect` of what to **REMOVE** — "Office (388)", "Transport (26)", "Heating & Energy (29)". The count is what makes the answer informed: "remove Office" and "remove 388 of 464 products" are the same choice, and only the second one is answerable.
   - **Never present a themed label whose membership the operator cannot see.** A shorthand ("only heating & energy", "the industrial range") is yours to **resolve**, never to accept as an answer — and if you offer a shorthand at all, **render its resolved membership inline in the same question** (which branches, which counts). Failure signature: the operator confirmed a phrase and the drop that followed touched branches the phrase never named — one run privately mapped "only heating & energy" to a 3-branch drop, cutting 464 → 29 products (94%) when the intent was "remove only Office", and cost a full adapt+reduce replay.
   - Record the answer as the **resolved set** (branch names + counts), not the operator's phrase. `reduce.md` confirms that tree again before pruning and applies its disproportion + feature-coupling gates there; **this** question is where the set is chosen.
...
9. **Run mode** (= questionnaire `R1`) — how the wizard proceeds once the answers are confirmed (ask it last; it's a deliberate choice, no silent default). Same work either way — only the check-in cadence differs (full behaviour: §3 of the wizard SKILL):
   - **autonomous** — runs steps 1–9 in one pass with no "continue?" check-ins; at any **reversible, in-project** decision point it picks the best option and records it in `.ai-dev/decision-log.md`. **This includes every questionnaire question left blank** — a blank is a delegated decision, not a deferred question. **Irrecoverable** actions (what `git checkout` cannot undo — DB/volume drops, `sudo`, deleting untracked files, any post-boot data deletion), an outsized-**magnitude** catalog reduction, and human-only prerequisites (start Docker, `/etc/hosts`, tokens) **still stop for explicit OK** (SKILL.md §3 hard-stops). Best for a repeat/experienced run, and the mode the questionnaire exists to serve.
...
fast-path; it is what stops a typo becoming a 40-minute wrong boot.)

**A blank `T2` (stores) gets its OWN explicit confirmation line, in both modes — it is the one default
that is never silently taken.** It resolves out of an **untouched** `deploy.dev.yml` and then cascades
into `data.mode: leave`, skipping `define-stores` and `project-data` entirely — and a store answer
inherited from a file the developer never edited is not an answer. State the resolved consequence in
...
`SKIPPED (data.mode=leave)` on a decision nobody ever said out loud.
...
standing_approval: null                 # or { scope: "<the class the developer pre-approved, in their own words>", granted: <timestamp> } — honoured only inside that scope; NEVER covers reset/clean-data/volume drops or any post-boot data deletion (SKILL.md §3)
