...
- **`key_translation.<locale>` — the attribute NAME/label shown on the PDP** ("Color", "Heat Recovery Output", "Body Material"). This is the one the first run missed: `key_translation.pl_PL` stayed the English copy `"Color"`, so the storefront showed English labels next to Polish values (`Heat Recovery Output: do 150 kW`). **It is display text, not a key — translate it.**
...
- **Per-locale files** (`glossary.<locale>.csv` — the shape `project-data` adapt produces): translate the `translation` column of the target locale's file, using the source locale's file as the source — `distinct` on `glossary.en_US.csv:translation` → map → `apply-translations glossary.<locale>.csv --source-column translation --target-column translation --map …` won't work column-to-column across files; instead extract the source values, map them, and apply the map onto the target file's `translation` column (same-column apply: source-column = target-column = `translation` — the map's `source` side carries the English values the copy still holds).
