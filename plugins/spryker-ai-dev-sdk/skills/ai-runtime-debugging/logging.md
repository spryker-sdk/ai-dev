...
| Log line delayed by minutes from Glue/Yves | `QueueHandler` async-writes via RabbitMQ; consumed by the queue worker | Either run the worker (`docker/sdk console queue:worker:start --stop-when-empty`), or write through Zed (synchronous handler), or use `file_put_contents` fallback |
