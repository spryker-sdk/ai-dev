...
flowchart TD
    ...
    subgraph P0["Phase 0 — Research FIRST 🔬 REQUIRED"]
        Q1{"Ask: research<br/>official docs?"}
        Q1 -- "yes" --> R1["Dispatch spryker-docs-research<br/>as subagent(s) via Agent tool<br/>— parallel when multi-concept —<br/>NEVER inline; merge briefs"]
        Q1 -- "no" --> R2
        R1 --> R2["Codebase facts — INLINE, always<br/>getSprykerModules ·<br/>getSprykerModuleMap ·<br/>getTransferStructureByName"]
        R2 --> MCP{"Tooling MCP<br/>connected?"}
    ...
    P3["Phase 3 — Next user story<br/>ONE AT A TIME<br/>canonical actor from actors.md<br/>+ endpoint resolved now (Phase 5 rules)"]
    P3 --> P3C{"Story CHECKPOINT<br/>story text + actor +<br/>affected endpoint — all three"}
    P3C -- "fix actor / fix endpoint / modify story" --> P3
    P3C -- "approve" --> P4

    P4["Phase 4 — Acceptance criteria<br/>2–4 Gherkin scenarios,<br/>presented ONE BY ONE"]
...
| Phase | Delegates to |
|---|---|
| 0 · docs (opt-in) | `Skill(spryker-docs-research)` — dispatched via the **Agent** tool as one or more subagents, parallel for multi-concept features, never inline |
| 0 · codebase facts | Spryker tooling MCP, inline: `getSprykerModules`, `getSprykerModuleMap`, `getTransferStructureByName` + `deploy.dev.yml` |
...
| [`actors.md`](actors.md) | The canonical actor set — Back Office user, Customer, Agent, Merchant user, Merchant Agent — where each operates, how each authenticates, the Merchant user vs. Merchant Agent scope distinction, and story phrasing. |
...
| [`template.md`](template.md) | The allowed PRD sections — Background, Goals, User Stories, NFRs, Success Metrics, Out of Scope, Dependencies. No Quality Gates, no Tasks, no priority tiers. |
...
- **Interactive checkpoints.** After Background & Goals, after EACH story, after EACH criterion, after
  NFRs, and before saving. Never batched, never a complete PRD in one pass.
