...
class CopyDataImportCsvWithMappingAiDevMcpToolPlugin extends AbstractPlugin implements AiDevMcpToolPluginInterface
{
    ...
    {
        return 'Copy and transform CSV data from source to target file with advanced mapping, filtering, and value replacement. Supports: 1:1 column mapping, 1:many expansion (duplicate value to multiple columns), filtering rows with AND/OR logic, value replacements, append/overwrite modes. Supports only relative paths. Parameters: sourceFilePath (string), targetFilePath (string), filters (JSON array, default []), columnMapping (JSON object, default {}), valueReplacements (JSON object, default {}), mode (string: "append"|"overwrite", default "append"), filterLogic (string: "AND"|"OR", default "AND"). Filter example: [{"column":"store","value":"US"}]. Mapping examples: 1:1: {"source_sku":"concrete_sku"}. 1:many: {"name":["name.de_DE","name.en_US"]} (copies name to both columns). Value replacement: {"store":{"EU":"AT","US":"US-EAST"}} (replaces EU with AT, US with US-EAST in store column). Use case: Copy US products, map columns, replace store codes: filters=[{"column":"store","value":"US"}], columnMapping={"sku":"concrete_sku"}, valueReplacements={"store":{"US":"US-EAST"}}.';
    }
